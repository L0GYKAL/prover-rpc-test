...
pub async fn prove_cairo_program() -> () { //Result<(), Box<dyn Error>>
    let mut client = StarknetProverClient::connect("http://192.168.204.3:10000").await.unwrap();
    let compiled_program = std::fs::read("example_cairo_program/fibonacci/target/dev/fibonacci_HelloStarknet.compiled_contract_class.json").unwrap();
